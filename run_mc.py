...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./mc", help="Where to store the final model.")
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        if args.context_file is not None:
            with open(args.context_file, newline='', encoding="utf-8") as jsonfile1:
                context = json.load(jsonfile1)

        if args.train_file is not None:
            with open(args.train_file, encoding='utf-8') as jsonfile2:
                train_data = json.load(jsonfile2)
            data_file = open('dataset/train_mc.csv', 'w', newline='', encoding='utf-8-sig')
        ...
        if args.validation_file is not None:
            with open(args.validation_file, encoding='utf-8') as jsonfile1:
                validation_data = json.load(jsonfile1)
            
            data_file = open('dataset/valid_mc.csv', 'w', newline='', encoding='utf-8-sig')
            csv_writer = csv.writer(data_file)
            header = ['id', 'sent1', 'ending0', 'ending1', 'ending2', 'ending3', 'label']
            csv_writer.writerow(header)
