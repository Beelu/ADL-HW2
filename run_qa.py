...
def main():
    ...
    if args.dataset_name is not None:
        ...
        if args.context_file is not None:
            with open(args.context_file, encoding="utf-8") as jsonfile1:
                context = json.load(jsonfile1)
        ...
        if args.train_file is not None:
            ...
            for data in train_data[:1000]:
                ...
                artical = context[artical_num]
                data_values = {'id':data['id'], 'context':artical, 'question':data['question'], 'answers':answer}
                data_for_json.append(data_values)
            with open('dataset/train_qa.json', 'w', encoding='utf-8') as f:
                json.dump(data_for_json, f, ensure_ascii=False)
            data_files["train"] = 'train_qa.json'
        ...
        if args.validation_file is not None:
            ...
            for data in valid_data[:1000]:
                ...
                data_values = {'id':data['id'], 'context':artical, 'question':data['question'], 'answers':answer}
                data_for_json.append(data_values)
            with open('dataset/valid_qa.json', 'w', encoding='utf-8') as f:
                json.dump(data_for_json, f, ensure_ascii=False)
            data_files["validation"] = 'valid_qa.json'

        # test data
        if args.test_file is not None:
            with open(args.test_file, encoding='utf-8') as jsonfile2:
                test_data = json.load(jsonfile2)
            with open("./pred_article.json", encoding='utf-8') as jsonfile3:
                all_article_num = json.load(jsonfile3)
            data_for_json = []
            for i, data in enumerate(test_data):
                answer = {"text":['asd'], "answer_start":[1]}
                artical_num = data['paragraphs'][all_article_num[i]]
                artical = context[artical_num]
                data_values = {'id':data['id'], 'context':artical, 'question':data['question'], 'answers':answer}
                data_for_json.append(data_values)
            with open('dataset/test_qa.json', 'w', encoding='utf-8') as f:
                json.dump(data_for_json, f, ensure_ascii=False)
    ...
    def prepare_train_features(examples):
        ...
        # 做tokenize跟一些padding，如果context字串太長會分割，而這會造成一個context可能有多個feature
        ...
        # 因為較長的context會有多個feature，所以我們需要一個map，將多個feature對應到正確的context
        sample_mapping = tokenized_examples.pop("overflow_to_sample_mapping")
        # The offset mappings will give us a map from token to character position in the original context. This will
        # help us compute the start_positions and end_positions.
        # 做QA問題時輸出是開始跟結束位置，因此我們在這裡有一個map將token對應到在原本context中的character position
